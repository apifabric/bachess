...
resourcename="bachess"  # lower case, only
resourcegroup="bachess_rg"
...
# git clone https://github.com/apilogicserver/bachess.git
# cd bachess
...
az group create --name bachess_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group bachess_rg --sku S1 --is-linux
az container create --resource-group bachess_rg --name bachess --image apilogicserver/bachess:latest --dns-name-label bachess --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//bachess.westus.azurecontainer.io:5656'
